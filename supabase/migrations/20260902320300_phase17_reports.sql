-- Phase 17: rooms do not quietly change what a report says.
--
-- `report_activity()`'s Discussion figure counts every comment in the period.
-- Before rooms, that meant correspondence logged against a record -- an issue,
-- a drawing, a matrix duty -- which is what the sentence claims. Room messages
-- are the same table, so without this the number silently becomes "comments
-- plus chat", a busy week in one room reads as a productive month across the
-- project, and a client document says so.
--
-- The count therefore excludes rooms, and rooms get a line of their own for
-- the internal audience alone. A client is told nothing about them: a report is
-- a document that leaves the building, and working conversation is not a fact
-- for an automated one -- the same judgement that withholds gone-quiet.
--
-- Nothing here needs a visibility clause of its own. `report_activity()` is an
-- invoker function, so a room the reader is not in is already absent from
-- every count it makes.
...
returns table (sort_order int, section text, headline text, detail text[])
...
begin
  ...
  --
  -- Correspondence against a record, which is what the sentence says. Room
  -- messages are the same table and are excluded here, or the figure quietly
  -- becomes "comments plus chat" and stops meaning what it claims.
  select count(*)::int into n1 from comments c
  where c.project_id = p_project and c.entity_type <> 'room'
    and c.created_at::date between v_start and v_end
    and (v_co is null
  ...
  -- -------------------------------------------------------------- rooms
  -- Internal only. A room is working conversation, and a stalled or a noisy
  -- one is a tone judgement rather than a fact for a document addressed to a
  -- client or a consultant -- the same reason gone-quiet is withheld from
  -- them. The contractor's own staff get it because they are the people who
  -- would act on it.
  if p_audience = 'internal' then
    select count(*)::int into n1 from comments c
    join chat_rooms r on r.id = c.entity_id
    where c.entity_type = 'room' and c.project_id = p_project
      and c.created_at::date between v_start and v_end;
    if n1 > 0 then
      select array_agg(x.line) into v_detail from (
        select r.name || ' — ' || count(*)::text || ' message'
      ...
      return query select 9, 'Rooms'::text,
        n1::text || ' message' || case when n1 = 1 then '' else 's' end
        || ' across the project''s rooms.', coalesce(v_detail, '{}');
    end if;
  end if;
end;
$$;
