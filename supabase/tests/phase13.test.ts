...
describe('what the client report does not say', () => {
  test('no fee or cashflow figure appears anywhere in it', async () => {
    ...
    // Unformatted: a money metric returns a number and `unit` says to render
    // it as currency, because a pound sign written into SQL arrives in the
    // database already corrupted when a migration is pasted through a client
    // that guesses the encoding.
    expect(internal).toMatch(/430000/)

    const client = await metricsText(w.admin, 'client')
    expect(client).not.toMatch(/fee/i)
    expect(client).not.toMatch(/£/)
    expect(client).not.toMatch(/430000|250000|180000/)
...
describe('a consultant report never reveals another company’s figures', () => {
  test('the fee figure is that company’s own, not the project’s', async () => {
    ...
    expect(mine).toMatch(/250000/)
    expect(mine).not.toMatch(/180000/)   // Northgate's
    expect(mine).not.toMatch(/430000/)   // the project total

    const theirs = await metricsText(w.admin, 'consultant', w.ngt)
    expect(theirs).toMatch(/180000/)
    expect(theirs).not.toMatch(/250000/)
  })
